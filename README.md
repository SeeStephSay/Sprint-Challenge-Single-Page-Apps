...
> Answer: 
Client-side routing can take longer to start the first time, but the time between views tends to be quicker, and it makes for smoother animations, too. Less data is processed each time, because the site isn't sending requests to the server every time a view updates on the screen. It does this by changing the state of the app, and rendering a different component, or bringing in different data from the server.
